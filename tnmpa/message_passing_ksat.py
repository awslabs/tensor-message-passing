...
class TwoNormBeliefPropagation(MessagePassingKsatSolver):
    ...
    def entropy_env(self, env):
        from scipy.linalg import svdvals  # , logm
        from scipy.stats import entropy

        mat_env = env.reshape(2, 2)
        # ee += - (mat_env @ logm(mat_env)).trace()
        s = svdvals(mat_env / mat_env.trace())
        return entropy(s * s)

    def two_renyi_entropy_bond(self, variable, clause_label):
        env1 = self.envs_tensors[variable][clause_label].reshape(2, 2)
        env2 = self.envs_tensors[clause_label][variable].reshape(2, 2)
    ...
    def entanglement_entropy(self, region):
        ...
        ee = 0.0
        for v in region:
            for c_label in self.envs_tensors[v].keys():
                c = self.instance.clauses[c_label]
                if not all(item in region for item in c.pos):
                    ee += self.entropy_env(self.envs_tensors[v][c_label])
                    ee += self.entropy_env(self.envs_tensors[c_label][v])
                else:
                    print(c_label)
        return ee

    def topological_entaglement_entropy(self, A, B, C):
        AB = np.concatenate((A, B))
        BC = np.concatenate((B, C))
        AC = np.concatenate((A, C))
        ABC = np.concatenate((A, B, C))

        Sa = self.entanglement_entropy(A)
        Sb = self.entanglement_entropy(B)
        Sc = self.entanglement_entropy(C)
        Sab = self.entanglement_entropy(AB)
        Sbc = self.entanglement_entropy(BC)
        Sac = self.entanglement_entropy(AC)
        Sabc = self.entanglement_entropy(ABC)
